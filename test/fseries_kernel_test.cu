...
int main(int argc, char *argv[]) {
    int nf1;
    if (argc < 2) {
        fprintf(stderr, "Usage: onedim_fseries_kernel_test nf1 [dim [tol [gpuversion [nf2 [nf3]]]]]\n"
                        "Arguments:\n"
    ...
    if (!gpu) {
        ...
        {
            checkCudaErrors(
                cudaMemcpy(d_fwkerhalf1, fwkerhalf1, sizeof(CUFINUFFT_FLT) * (nf1 / 2 + 1), cudaMemcpyHostToDevice));
            if (dim > 1)
                checkCudaErrors(cudaMemcpy(d_fwkerhalf2, fwkerhalf2, sizeof(CUFINUFFT_FLT) * (nf2 / 2 + 1),
                                           cudaMemcpyHostToDevice));
            if (dim > 2)
                checkCudaErrors(cudaMemcpy(d_fwkerhalf3, fwkerhalf3, sizeof(CUFINUFFT_FLT) * (nf3 / 2 + 1),
                                           cudaMemcpyHostToDevice));
        }
    ...
    checkCudaErrors(
        cudaMemcpy(fwkerhalf1, d_fwkerhalf1, sizeof(CUFINUFFT_FLT) * (nf1 / 2 + 1), cudaMemcpyDeviceToHost));
    if (dim > 1)
        checkCudaErrors(
            cudaMemcpy(fwkerhalf2, d_fwkerhalf2, sizeof(CUFINUFFT_FLT) * (nf2 / 2 + 1), cudaMemcpyDeviceToHost));
    if (dim > 2)
        checkCudaErrors(
            cudaMemcpy(fwkerhalf3, d_fwkerhalf3, sizeof(CUFINUFFT_FLT) * (nf3 / 2 + 1), cudaMemcpyDeviceToHost));
