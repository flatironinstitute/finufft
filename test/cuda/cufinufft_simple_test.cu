...
// plan path on identical inputs and require the device outputs to agree to a
// small relative L2 error (atomic-add non-determinism prevents bit-exactness).
// Both float and double precision are exercised.
...
// Per-precision constants in one place: complex type, label, requested
// transform tolerance (REQ_TOL), and allowed simple-vs-plan relative error
// (CHECK_RTOL, loose for float to cover type-3 error accumulation).
template<typename T> struct Traits;
template<> struct Traits<float> {
  using cuc = cuFloatComplex;
  static constexpr const char *label = "float";
  static constexpr float REQ_TOL = 1e-5f;
  static constexpr float CHECK_RTOL = 5e-3f;
};
template<> struct Traits<double> {
  using cuc = cuDoubleComplex;
  static constexpr const char *label = "double";
  static constexpr double REQ_TOL = 1e-9;
  static constexpr double CHECK_RTOL = 1e-9;
};
...
// One random test problem (NU points, type-3 targets, input data) sized for
// (dim, type); the same Fixture feeds run_plan() and run_simple() so both see
// identical inputs.
...
template<typename T> Fixture<T> make_fixture(int dim, int type, std::mt19937 &rng) {
  ...
  std::uniform_real_distribution<T> uni_s(T(-2), T(2)); // type-3 freq targets, small band
  std::uniform_real_distribution<T> uni_c(T(-1), T(1)); // strengths/coeffs (Re,Im)
  f.x.resize(f.M);
...
template<typename T> std::vector<std::complex<T>> run_plan(const Fixture<T> &f) {
  ...
  if constexpr (std::is_same_v<T, float>) {
    ...
    CHECK_RET(cufinufftf_makeplan(f.type, f.dim, nmodes, 1, 1, Traits<T>::REQ_TOL, &plan,
                                  nullptr),
              "makeplan");
    ...
    cufinufft_plan plan{};
    CHECK_RET(cufinufft_makeplan(f.type, f.dim, nmodes, 1, 1, Traits<T>::REQ_TOL, &plan,
                                 nullptr),
              "makeplan");
...
template<typename T> int compare_one(int dim, int type, std::mt19937 &rng) {
  ...
  // atomic-add non-determinism separates the two paths; CHECK_RTOL bounds it.
  const T rerr = relerrtwonorm(via_p.size(), via_p.data(), via_s.data());
  if (rerr > Traits<T>::CHECK_RTOL) {
    std::fprintf(stderr, "mismatch %s dim=%d type=%d (n=%zu): rel L2 err %g (tol %g)\n",
                 Traits<T>::label, dim, type, via_p.size(), double(rerr),
                 double(Traits<T>::CHECK_RTOL));
    return 1;
  }
