...
template<typename T> int compare_one(int dim, int type, std::mt19937 &rng) {
  ...
  // The two paths run the same algorithm; only atomic-add non-determinism
  // separates them. CHECK_RTOL (see Traits) is the allowed relative error.
  const T rtol = Traits<T>::CHECK_RTOL;
