...
Error model (matches C++ lowest_sigma in kernel.cpp):
...
# Model coefficients — must match kernel.cpp lowest_sigma.
...
# ---------------------------------------------------------------------------
# Model (matches C++ lowest_sigma in kernel.cpp)
# ---------------------------------------------------------------------------
def _invert_kernel_sigma(tol, ns, dim):
    ...
    if tol <= 0:
        return MAX_CHECK_SIGMA
    if tol >= tolfac:
        return 1.0 + 1e-6
    u = np.log(tolfac / tol) / ((ns - 1.0) * PI)
    if u >= 1.0:
        return MAX_CHECK_SIGMA
    return min(1.0 / (1.0 - u * u), MAX_CHECK_SIGMA)
...
def _mreineck_sigma_min(tol, ns, dim, type_, eps_mach, gridlen):
    ...
    if mreineck_tol(MAX_CHECK_SIGMA) > tol:
        return MAX_CHECK_SIGMA + 1.0
    lo, hi = 1.01, MAX_CHECK_SIGMA
    for _ in range(60):
        mid = 0.5 * (lo + hi)
