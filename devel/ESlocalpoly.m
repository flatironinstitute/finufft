...
for i=1:w   % loop over interval numbers to test
  ...
  for j=1:numel(ms), m=ms(j);
    ...
    V = ones(4*m,p);
    for k=2:p, V(:,k) = V(:,k-1) .* z; end
    c(:,j) = V\f(xi+z*h/2);              % solve
    % test sub norm over the real int
    er(i,j) = max(abs(f(xi+t*h/2) - polyval(c(end-p+pe:-1:1,j),t)));
  end
...
% w=15: pe=17 to beat 1e-14
% w=13: pe=15 to beat 1e-12.
% w=7:  pe=11  be well below 1e-6
% w=4:  pe=8        "        1e-3
% w=3:  pe=7
