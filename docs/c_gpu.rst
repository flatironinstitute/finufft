...
interface, performed internally. Type-3 calls also take frequency-target arrays (``s`` for 1D,
...
This is a convenience layer over the 4-step plan interface, which remains the
primary API and is unchanged. The simple interface is intended for cases where
the plan would otherwise be used exactly once; when the same NU points are
reused across several transforms, use the 4-step plan interface below to avoid
repeating the plan setup.
