...
* The kernel function in spreadinterp is evaluated via piecewise-polynomial approximation (Horner's rule). Both libraries fit those coefficients at plan time, in ``finufft::kernel::fit_horner_coeffs`` (``include/finufft_common/kernel.h``), so no coefficient table is generated or hardwired and any kernel formula and upsampling factor works. The fit always runs in double; the plan's precision sets only the storage precision of the table. ``finufft::common::MAX_NC<T>`` caps the panel degree per storage precision, by accuracy budget rather than by where the fit saturates; it binds only in single precision. The CPU plan reads them with an explicitly SIMD-vectorized evaluator (``evaluate_kernel_vector``) and the GPU plan with ``eval_kernel_vec_horner``; ``finufft::kernel::evaluate_kernel_horner`` is the scalar reference both must match. The MATLAB generators left in ``devel/`` are the pre-2026 vintage and no longer feed the build.
...
* **Installing MWrap**. This is needed only for experts to rebuild the matlab/octave interfaces.
  ...
  Every ``matlab/*.m`` file, ``matlab/finufft.cpp``, ``matlab/cufinufft.cu`` and
  ``docs/matlab*help.doc`` is generated by ``make mex``. Never hand-edit one: edit
  ``matlab/*.mw``, ``matlab/*.docsrc`` or ``matlab/*.docbit`` and regenerate. To
  deprecate an option, remove the field from the ``.docbit``, warn via
  ``mexWarnMsgIdAndTxt`` in the ``.mw`` opts parser, and regenerate.
