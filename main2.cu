                       ...
                       double *h_ky, double *h_c, int bin_size_x, int bin_size_y)
{
  ...
  if(numbins[0]*numbins[1] < 1024) // 1024 is the maximum #threads per block 
    BinsStartPts_2d<<<1, numbins[0]*numbins[1], numbins[0]*numbins[1]>>>(M,numbins[0]*numbins[1],
  ...
  for(int j=0; j<numbins[1]; j++){
    for(int i=0; i<numbins[0]; i++){
      cout << " bin["<<i<<","<<j<<"] = "<<h_binstartpts[i+j*numbins[0]];
    }
    cout<<endl;
  }
  cout << h_binstartpts[numbins[0]*numbins[1]] << endl;
