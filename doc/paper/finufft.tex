...
estimate which applies to smaller frequencies,
has explicit $\xi$ and $\freq$ dependence,
but (because of its second term) fails to be summable in $\xi$.
...
\begin{lem}
  ...
  the \FT\ of the ES kernel obeys
  \be
  |\hat\phi(\xi)| \; \le \; e^{-\freq} \left( C_1 \frac{\freq^2}{\xi^2}
  + C_2\frac{1}{\xi} \right)
  ~.
  \label{EShat4}
  \ee
...
\begin{proof}
  ...
  From \eqref{ES} and \eqref{FT} we have
  $$
  e^\freq \hat\phi(\xi) = \int_{-1}^1 e^{\freq\sqrt{1-z^2} + i\xi z}dz
  ~,
  $$
  and we again fix $\xi>0$ and deform the contour into the upper half plane,
  ...
  $I_2$ be the integral along the semicircle defined in \eqref{circ},
  and $I_3$ be the integral along the real axis from $R$ to $1$.
  Then $e^\freq \hat\phi(\xi) = I_1+I_2+I_3$.
  Note that the branch cuts of the integrand are taken to lie below the
  real axis.
...
\begin{rmk}[Comparison to \KB\ kernel error bounds]
  ...
  Note that \eqref{KBerr} has the constant explicit.
