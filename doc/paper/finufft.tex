...
% NOtes on KB:  6/11/17
% Fourmont 03, Potts, other germans, use backwards KB pair,
% ie I_0 on the k side, so there is no aliasing error, just the spreading
% func has exp small tails which do not die.
% Error estimate then involves the spreading sum over these tails
% (Fourmont).
%  Fessler 03 uses I_0 on the x spreading side, so spreading kernel cpt
% support, and the error is all due to aliasing.
% (Fessler's weird sqrt() factor infront goes away for m=0 order case).
...
% Fessler uses 2.35  - maybe we should try that?
