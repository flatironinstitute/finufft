...
%  ``the Fourier transform of the exponential of a semicircle tends to the exponential of a semicircle.''
...
For convenience we first review a known formula
...
~, \qquad k\in\KK,
$$
where, using Poisson summation \eqref{pois} with $\theta=kh$,
$$
g_k(x) :=  p_k \sum_{l=0}^{n-1} e^{2\pi i l k/n}
\ppsi(lh-x) - e^{ikx}
\; =\;
n p_k \sum_{m\in\ZZ} \hat\psi(k+mn) e^{i(k+mn)x} - e^{ikx}
~.
$$
Choosing $p_k$ as in \eqref{pk1} thus kills exactly the contributions from
...
\ee
Since $|k|\le N/2$,
error is thus controlled by the sum of the tails of $\hat\psi$
...
A simple way to get such uniform bounds from \eqref{gkx} is to discard phase
information, and choose over $k$
the worst-case numerator and denominator, giving
\be
\eps = \max_{|k|\le N/2}
\|g_k\|_\infty =
\max_{|k|\le N/2} \frac{1}{|\hat\psi(k)|}\sum_{m\neq 0} |\hat\psi(k+mn)|
\le
...
\ee
\begin{rmk}
  Estimation of $\eps$ in the case of the \KB\ kernel \eqref{KB}
  fails using \eqref{epsest}, since the tail of the sinc function in
  \eqref{KBhat}, decaying like $k^{-1}$, is not absolutely convergent.
  In this case, rigorous estimates,
  ...
  The results are summarized in \cite[Sec.~4]{fourmont} and used for the
  estimates in \cite[App.~C]{nfft}.
\end{rmk}
