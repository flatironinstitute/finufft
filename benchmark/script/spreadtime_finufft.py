...
def main():
	...
	#f= open("../results/finufft_spread_d_2_0814.out","w")
	f= open("../test.out","w")
	f.write("finufft spread:\n")
		...
		for t,tol in enumerate(tol_totry):
			for n,nf1 in enumerate(nf1_totry):
				...
				for nn in range(reps):
					tt = 0.0
					output=subprocess.check_output(["./spreadtestnd_mel",str(nupts_distr),str(nf1/2),str(nf1/2),str(M), \
	...
	np.save('../results/finufft_spread_d_2_0814.npy', t_gpuspread_5)
