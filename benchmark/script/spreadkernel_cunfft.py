...
def main():
	...
	f= open("../results/cunfft_spread_s_2_0812.out","w")
	f.write("cunfft spread (clustered nupts):\n")
	...
	for t,tol in enumerate(tol_totry):
		...
		subprocess.call(["cmake", "..",  "-DCUT_OFF="+str(cutoff), "-DCUNFFT_DOUBLE_PRECISION=OFF", "-DMILLI_SEC=ON", "-DMEASURED_TIMES=ON", \
		...
		for d,density in enumerate(density_totry):
			for n,nf1 in enumerate(nf1_totry):
				...
				for nn in range(reps):
					...
					tt+= float(find_between(output, "spread", "ms"))
	...
	np.save('../results/cunfft_spread_s_2_0812.npy', t_gpuspread_5)
