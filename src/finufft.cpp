 ...
 */
{
  ...
  for (int n = 0; n < q; ++n) {      // set up nodes z_n and vals f_n
    ...
    a[n] = -exp(2 * PI * IMA * (T)z[n] / (T)nf);          // phase winding rates
  }
...
template<typename T>
static int spreadinterpSortedBatch(int batchSize, FINUFFT_PLAN p, std::complex<T> *cBatch)
/*
...
template<typename T>
static int deconvolveBatch(int batchSize, FINUFFT_PLAN p, std::complex<T> *fkBatch)
/*
  ...
  if (p->type != 3) { // --------------------- TYPE 1,2 EXEC ------------------
    ...
    for (int b = 0; b * p->batchSize < p->ntrans; b++) { // .....loop b over batches
      ...
      if (p->type == 1) { // type 1: spread NU pts p->X, weights cj, to fw grid
        spreadinterpSortedBatch<FLT>(thisBatchSize, p, cjb);
        t_sprint += timer.elapsedsec();
      } else { //  type 2: amplify Fourier coeffs fk into 0-padded fw
        deconvolveBatch<FLT>(thisBatchSize, p, fkb);
...
#pragma omp parallel for num_threads(p->opts.nthreads) // or p->batchSize?
      ...
      spreadinterpSortedBatch<FLT>(thisBatchSize, p, p->CpBatch); // p->X are primed
