...
struct Interp2DDispatcher {
  ...
  void operator()(const cufinufft_plan_t<T> &d_plan, cuda_complex<T> *c, const cuda_complex<T> *fw, int blksize) const {
    ...
    case 1:
      return cuinterp_nuptsdriven<T, 2, ns>(d_plan, c, fw, blksize);
    case 2:
      return cuinterp_subprob<T, 2, ns>(d_plan, c, fw, blksize);
    default:
...
template void cuinterp2d<float>(const cufinufft_plan_t<float> &d_plan, cuda_complex<float> *c, const cuda_complex<float> *fw, int blksize);
template void cuinterp2d<double>(const cufinufft_plan_t<double> &d_plan, cuda_complex<double> *c, const cuda_complex<double> *fw, int blksize);
