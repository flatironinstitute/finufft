...
template void cuspread2d<float>(const cufinufft_plan_t<float> &d_plan,
                                const cuda_complex<float> *c, cuda_complex<float> *fw,
                                int blksize);
template void cuspread2d<double>(const cufinufft_plan_t<double> &d_plan,
...
template<typename T> void cuspread2d_prop(cufinufft_plan_t<T> &d_plan) {
  ...
  if (d_plan.opts.gpu_method == 2) cuspread_subprob_and_OD_prop<T, 2>(d_plan);
  if (d_plan.opts.gpu_method == 3) cuspread_subprob_and_OD_prop<T, 2>(d_plan);
}
