...
*/
{
  ...
  for (int i = 0; i * d_plan->batchsize < d_plan->ntransf; i++) {
    ...
    d_plan->fk  = d_fk + i * d_plan->batchsize * d_plan->mstu[0] * d_plan->mstu[1];  // so deconvolve will write into user output f
...
*/
{
  ...
  for (int i = 0; i * d_plan->batchsize < d_plan->ntransf; i++) {
    int blksize = std::min(d_plan->ntransf - i * d_plan->batchsize, d_plan->batchsize);
    d_plan->c  = d_c + i * d_plan->batchsize * d_plan->M;
    d_plan->fk = d_fk + i * d_plan->batchsize * d_plan->mstu[0] * d_plan->mstu[1];
...
static void cufinufft2d3_exec(cuda_complex<T> *d_c, cuda_complex<T> *d_fk,
  ...
  */
  ...
  for (int i = 0; i * d_plan->batchsize < d_plan->ntransf; i++) {
    ...
    cufinufft2d2_exec<T>(d_fkstart, d_plan->fw, d_plan->t2_plan);
    // Step 3: deconvolve
