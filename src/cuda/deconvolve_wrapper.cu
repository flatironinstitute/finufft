...
template void cufinufft_plan_t<float>::deconvolve(
    cuda_complex<float> *fw, cuda_complex<float> *fk, int blksize) const;
template void cufinufft_plan_t<double>::deconvolve(
    cuda_complex<double> *fw, cuda_complex<double> *fk, int blksize) const;
