...
struct nufft_opts {   // see common/finufft_default_opts() for defaults
  ...
  int spread_kerevalmeth; // "     spread_opts, 0: exp(sqrt()), 1: Horner ppval
  ...
  int fftw;           // 0:FFTW_ESTIMATE, or 1:FFTW_MEASURE for faster rerun
  int modeord;        // 0: CMCL-style increasing mode ordering, or
                      // 1: FFT-style mode ordering (affects type-1,2 only
  FLT upsampfac;      // upsampling ratio sigma, default is 2.0
};
