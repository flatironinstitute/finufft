...
int cuspread1d(cufinufft_opts &opts, cufinufft_plan* d_plan)
{
	...
	if(opts.pirange){
		...
		cudaEventSynchronize(stop);
		cudaEventElapsedTime(&milliseconds, start, stop);
		printf("[time  ]\tRescaleXY_1d\t\t %.3g ms\n", milliseconds);
#endif
	}
	...
	switch(opts.method)
	{
		...
		// 	{
		// 		cudaEventRecord(start);
		// 		ier = cuspread1d_subprob(nf1, nf2, fw_width, M, opts, d_plan);
		// 		if(ier != 0 ){
...
int cuspread1d_idriven(int nf1, int nf2, int fw_width, int M, const cufinufft_opts opts, cufinufft_plan *d_plan)
{
	...
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("[time  ] \tKernel Spread_1d_Idriven \t%.3g ms\n", milliseconds);
#endif
	return 0;
}

// int cuspread1d_subprob(int nf1, int nf2, int fw_width, int M, const cufinufft_opts opts, cufinufft_plan *d_plan)
// {
// 	cudaEvent_t start, stop;
// 	cudaEventCreate(&start);
// 	cudaEventCreate(&stop);
...
// 	}
//
// 	Spread_1d_Subprob<<<totalnumsubprob, 256, sharedplanorysize>>>(d_kx, d_ky, d_c,
// 			d_fw, M, ns, nf1, nf2,
...
// 	cudaEventSynchronize(stop);
// 	cudaEventElapsedTime(&milliseconds, start, stop);
// 	printf("[time  ] \tKernel Spread_1d_Subprob_V2 \t\t%.3g ms\n", milliseconds);
// #endif
// 	return 0;
// }
