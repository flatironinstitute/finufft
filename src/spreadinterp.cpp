...
static inline void TEMPLATE(set_kernel_args,T)(T *args, T x, const TEMPLATE(spread_opts,T)& opts);
static inline void TEMPLATE(evaluate_kernel_vector,T)(T *ker, T *args, const TEMPLATE(spread_opts,T)& opts, const int N);
static inline void TEMPLATE(eval_kernel_vec_Horner,T)(T *ker, const T z, const int w, const TEMPLATE(spread_opts,T) &opts);
void TEMPLATE(interp_line,T)(T *out,T *du, T *ker,BIGINT i1,BIGINT N1,int ns);
void TEMPLATE(interp_square,T)(T *out,T *du, T *ker1, T *ker2, BIGINT i1,BIGINT i2,BIGINT N1,BIGINT N2,int ns);
void TEMPLATE(interp_cube,T)(T *out,T *du, T *ker1, T *ker2, T *ker3,
		 BIGINT i1,BIGINT i2,BIGINT i3,BIGINT N1,BIGINT N2,BIGINT N3,int ns);
void TEMPLATE(spread_subproblem_1d,T)(BIGINT N1,T *du0,BIGINT M0,T *kx0,T *dd0,
			  const TEMPLATE(spread_opts,T)& opts);
void TEMPLATE(spread_subproblem_2d,T)(BIGINT N1,BIGINT N2,T *du0,BIGINT M0,
			  T *kx0,T *ky0,T *dd0,const TEMPLATE(spread_opts,T)& opts);
void TEMPLATE(spread_subproblem_3d,T)(BIGINT N1,BIGINT N2,BIGINT N3,T *du0,BIGINT M0,
			  T *kx0,T *ky0,T *kz0,T *dd0,
			  const TEMPLATE(spread_opts,T)& opts);
void TEMPLATE(add_wrapped_subgrid,T)(BIGINT offset1,BIGINT offset2,BIGINT offset3,
			 BIGINT size1,BIGINT size2,BIGINT size3,BIGINT N1,
			 BIGINT N2,BIGINT N3,T *data_uniform, T *du0);
void TEMPLATE(bin_sort_singlethread,T)(BIGINT *ret, BIGINT M, T *kx, T *ky, T *kz,
	      BIGINT N1,BIGINT N2,BIGINT N3,int pirange,
	      double bin_size_x,double bin_size_y,double bin_size_z, int debug);
void TEMPLATE(bin_sort_multithread,T)(BIGINT *ret, BIGINT M, T *kx, T *ky, T *kz,
	      BIGINT N1,BIGINT N2,BIGINT N3,int pirange,
	      double bin_size_x,double bin_size_y,double bin_size_z, int debug);
void TEMPLATE(get_subgrid,T)(BIGINT &offset1,BIGINT &offset2,BIGINT &offset3,BIGINT &size1,
		 BIGINT &size2,BIGINT &size3,BIGINT M0,T* kx0,T* ky0,
		 T* kz0,int ns, int ndims);


int TEMPLATE(spreadinterp,T)(
        BIGINT N1, BIGINT N2, BIGINT N3, T *data_uniform,
        BIGINT M, T *kx, T *ky, T *kz, T *data_nonuniform,
...
*/
{
  int ier_spreadcheck = TEMPLATE(spreadcheck,T)(N1, N2, N3, M, kx, ky, kz, opts);
  if (ier_spreadcheck != 0) return ier_spreadcheck;

  BIGINT* sort_indices = (BIGINT*)malloc(sizeof(BIGINT)*M);
  int did_sort = TEMPLATE(indexSort,T)(sort_indices, N1, N2, N3, M, kx, ky, kz, opts);
  int ier_spread = TEMPLATE(spreadwithsortidx,T)(sort_indices, N1, N2, N3, data_uniform,
...
#ifndef ONEDEF_SPREADINTERP
#define ONEDEF_SPREADINTERP
...
*/
{
  ...
  if (opts.chkbnds) {
    ...
    for (BIGINT i=0; i<M; ++i) {
      T x=RESCALE(kx[i],N1,opts.pirange);  // this includes +-1 box folding
    ...
    if (ndims>1)
      for (BIGINT i=0; i<M; ++i) {
        T y=RESCALE(ky[i],N2,opts.pirange);
    ...
    if (ndims>2)
      for (BIGINT i=0; i<M; ++i) {
        T z=RESCALE(kz[i],N3,opts.pirange);
...
*/
{
  ...
  if (opts.sort==1 || (opts.sort==2 && better_to_sort)) {
    ...
    if (sort_nthr==1)
      TEMPLATE(bin_sort_singlethread,T)(sort_indices,M,kx,ky,kz,N1,N2,N3,opts.pirange,bin_size_x,bin_size_y,bin_size_z,sort_debug);
    else
      TEMPLATE(bin_sort_multithread,T)(sort_indices,M,kx,ky,kz,N1,N2,N3,opts.pirange,bin_size_x,bin_size_y,bin_size_z,sort_debug);
    if (opts.debug) 
...
#pragma omp parallel for schedule(dynamic,1)
      for (int isub=0; isub<nb; isub++) {    // Main loop through the subproblems
        ...
        T *kx0=(T*)malloc(sizeof(T)*M0), *ky0=NULL, *kz0=NULL;
        if (N2>1)
          ky0=(T*)malloc(sizeof(T)*M0);
        if (N3>1)
          kz0=(T*)malloc(sizeof(T)*M0);
        T *dd0=(T*)malloc(sizeof(T)*M0*2);    // complex strength data
        ...
        TEMPLATE(get_subgrid,T)(offset1,offset2,offset3,size1,size2,size3,M0,kx0,ky0,kz0,ns,ndims);  // sets offsets and sizes
        ...
        T *du0=(T*)malloc(sizeof(T)*2*size1*size2*size3); // complex
        
        // Spread to subgrid without need for bounds checking or wrapping
        if (!(opts.flags & TF_OMIT_SPREADING)) {
          if (ndims==1)
            TEMPLATE(spread_subproblem_1d,T)(size1,du0,M0,kx0,dd0,opts);
          else if (ndims==2)
            TEMPLATE(spread_subproblem_2d,T)(size1,size2,du0,M0,kx0,ky0,dd0,opts);
          else
            TEMPLATE(spread_subproblem_3d,T)(size1,size2,size3,du0,M0,kx0,ky0,kz0,dd0,opts);
	}
        
#pragma omp critical
        {  // do the adding of subgrid to output; only here threads cannot clash
          if (!(opts.flags & TF_OMIT_WRITE_TO_GRID))
            TEMPLATE(add_wrapped_subgrid,T)(offset1,offset2,offset3,size1,size2,size3,N1,N2,N3,data_uniform,du0);
        }  // end critical block
...
int TEMPLATE(interpSorted,T)(BIGINT* sort_indices,BIGINT N1, BIGINT N2, BIGINT N3, 
  ...
  int ndims = ndims_from_Ns(N1,N2,N3);
  int ns=opts.nspread;          // abbrev. for w, kernel width
  T ns2 = (T)ns/2;          // half spread width, used as stencil shift
...
#define CHUNKSIZE 16     // Chunks of Type 2 targets (Ludvig found by expt)
    ...
    T kernel_args[3*MAX_NSPREAD];
    T kernel_values[3*MAX_NSPREAD];
    T *ker1 = kernel_values;
    T *ker2 = kernel_values + ns;
    T *ker3 = kernel_values + 2*ns;       
...
#pragma omp for schedule(dynamic) // assign threads to NU targ pts:
    ...
    for (int ibuf=0; ibuf<bufsize; ibuf++) {
      T xj = xjlist[ibuf];
      T yj = (ndims > 1) ? yjlist[ibuf] : 0;
      T zj = (ndims > 2) ? zjlist[ibuf] : 0;

      T *target = outbuf+2*ibuf;
      ...
      T x1=(T)i1-xj;           // shift of ker center, in [-w/2,-w/2+1]
      T x2= (ndims > 1) ? (T)i2-yj : 0 ;
      T x3= (ndims > 2)? (T)i3-zj : 0;

      // eval kernel values patch and use to interpolate from uniform data...
      if (!(opts.flags & TF_OMIT_SPREADING)) {

	  if (opts.kerevalmeth==0) {               // choose eval method
	    TEMPLATE(set_kernel_args,T)(kernel_args, x1, opts);
	    if(ndims > 1)  TEMPLATE(set_kernel_args,T)(kernel_args+ns, x2, opts);
	    if(ndims > 2)  TEMPLATE(set_kernel_args,T)(kernel_args+2*ns, x3, opts);
	    
	    TEMPLATE(evaluate_kernel_vector,T)(kernel_values, kernel_args, opts, ndims*ns);
	  }

	  else{
	    TEMPLATE(eval_kernel_vec_Horner,T)(ker1,x1,ns,opts);
	    if (ndims > 1) TEMPLATE(eval_kernel_vec_Horner,T)(ker2,x2,ns,opts);  
	    if (ndims > 2) TEMPLATE(eval_kernel_vec_Horner,T)(ker3,x3,ns,opts);
	  }

	  switch(ndims){
	  case 1:
	    TEMPLATE(interp_line,T)(target,data_uniform,ker1,i1,N1,ns);
	    break;
	  case 2:
	    TEMPLATE(interp_square,T)(target,data_uniform,ker1,ker2,i1,i2,N1,N2,ns);
	    break;
	  case 3:
	    TEMPLATE(interp_cube,T)(target,data_uniform,ker1,ker2,ker3,i1,i2,i3,N1,N2,N3,ns);
	    break;
...
*/
{
  ...
  if (opts.spread_direction==1)  // ========= direction 1 (spreading) =======
    TEMPLATE(spreadSorted,T)(sort_indices, N1, N2, N3, data_uniform, M, kx, ky, kz, data_nonuniform, opts, did_sort);
    
  else          // ================= direction 2 (interpolation) ===========
    TEMPLATE(interpSorted,T)(sort_indices, N1, N2, N3, data_uniform, M, kx, ky, kz, data_nonuniform, opts, did_sort);
...
// Also sets all default options in spread_opts,T. See cnufftspread.h for opts.
...
// Returns: 0 success, >0 failure (see error codes in utils.h)
{
  ...
  // (T)10.0 is cosmetic: sng & dbl desire same ns for bdry cases eps=10^{-n}
  if (upsampfac!=(T)2.0)           // override ns when custom sigma
...
static inline void TEMPLATE(eval_kernel_vec_Horner,T)(T *ker, const T x, const int w,
					  const TEMPLATE(spread_opts,T) &opts)
...
void TEMPLATE(interp_line,T)(T *target,T *du, T *ker,BIGINT i1,BIGINT N1,int ns)
...
void TEMPLATE(interp_square,T)(T *target,T *du, T *ker1, T *ker2, BIGINT i1,BIGINT i2,BIGINT N1,BIGINT N2,int ns)
...
void TEMPLATE(interp_cube,T)(T *target,T *du, T *ker1, T *ker2, T *ker3,
		 BIGINT i1,BIGINT i2,BIGINT i3, BIGINT N1,BIGINT N2,BIGINT N3,int ns)
...
void TEMPLATE(spread_subproblem_1d,T)(BIGINT N1,T *du,BIGINT M,
			  T *kx,T *dd,
			  const TEMPLATE(spread_opts,T)& opts)
...
*/
{
  ...
  for (BIGINT i=0;i<2*N1;++i)
    du[i] = 0.0;
  T kernel_args[MAX_NSPREAD];
  T ker[MAX_NSPREAD];
  for (BIGINT i=0; i<M; i++) {           // loop over NU pts
    T re0 = dd[2*i];
    T im0 = dd[2*i+1];
    BIGINT i1 = (BIGINT)std::ceil(kx[i] - ns2);
    T x1 = (T)i1 - kx[i];            // x1 in [-w/2,-w/2+1]
    if (opts.kerevalmeth==0) {
      TEMPLATE(set_kernel_args,T)(kernel_args, x1, opts);
      TEMPLATE(evaluate_kernel_vector,T)(ker, kernel_args, opts, ns);
    } else
      TEMPLATE(eval_kernel_vec_Horner,T)(ker,x1,ns,opts);
    // critical inner loop: 
    BIGINT j=i1;
    for (int dx=0; dx<ns; ++dx) {
      T k = ker[dx];
...
void TEMPLATE(spread_subproblem_2d,T)(BIGINT N1,BIGINT N2,T *du,BIGINT M,
			  T *kx,T *ky,T *dd,
			  const TEMPLATE(spread_opts,T)& opts)
 ...
 */
{
  ...
  for (BIGINT i=0; i<M; i++) {           // loop over NU pts
    ...
    if (opts.kerevalmeth==0) {
      TEMPLATE(set_kernel_args,T)(kernel_args, x1, opts);
      TEMPLATE(set_kernel_args,T)(kernel_args+ns, x2, opts);
      TEMPLATE(evaluate_kernel_vector,T)(kernel_values, kernel_args, opts, 2*ns);
    } else {
      TEMPLATE(eval_kernel_vec_Horner,T)(ker1,x1,ns,opts);
      TEMPLATE(eval_kernel_vec_Horner,T)(ker2,x2,ns,opts);
    }
    // Combine kernel with complex source value to simplify inner loop
    T ker1val[2*MAX_NSPREAD];
    for (int i = 0; i < ns; i++) {
...
void TEMPLATE(spread_subproblem_3d,T)(BIGINT N1,BIGINT N2,BIGINT N3,T *du,BIGINT M,
			  T *kx,T *ky,T *kz,T *dd,
			  const TEMPLATE(spread_opts,T)& opts)
 ...
 */
{
  ...
  for (BIGINT i=0; i<M; i++) {           // loop over NU pts
    ...
    if (opts.kerevalmeth==0) {
      TEMPLATE(set_kernel_args,T)(kernel_args, x1, opts);
      TEMPLATE(set_kernel_args,T)(kernel_args+ns, x2, opts);
      TEMPLATE(set_kernel_args,T)(kernel_args+2*ns, x3, opts);
      TEMPLATE(evaluate_kernel_vector,T)(kernel_values, kernel_args, opts, 3*ns);
    } else {
      TEMPLATE(eval_kernel_vec_Horner,T)(ker1,x1,ns,opts);
      TEMPLATE(eval_kernel_vec_Horner,T)(ker2,x2,ns,opts);
      TEMPLATE(eval_kernel_vec_Horner,T)(ker3,x3,ns,opts);
    }
    // Combine kernel with complex source value to simplify inner loop
    T ker1val[2*MAX_NSPREAD];
    for (int i = 0; i < ns; i++) {
...
*/
{
  ...
  for (int dz=0; dz<size3; dz++) {       // use ptr lists in each axis
    ...
    for (int dy=0; dy<size2; dy++) {
      ...
      T *out = data_uniform + 2*oy;
      T *in  = du0 + 2*size1*(dy + size2*dz);   // ptr to subgrid array
...
void TEMPLATE(bin_sort_multithread,Y)(BIGINT *ret, BIGINT M, T *kx, T *ky, T *kz,
	      BIGINT N1,BIGINT N2,BIGINT N3,int pirange,
	      double bin_size_x,double bin_size_y,double bin_size_z, int debug)
...
void TEMPLATE(get_subgrid,T)(BIGINT &offset1,BIGINT &offset2,BIGINT &offset3,BIGINT &size1,BIGINT &size2,BIGINT &size3,BIGINT M,T* kx,T* ky,T* kz,int ns,int ndims)
...
*/
{
  ...
  T min_kx,max_kx;
  TEMPLATE(arrayrange,T)(M,kx,&min_kx,&max_kx);
  ...
  if (ndims>1) {
    T min_ky,max_ky;
    TEMPLATE(arrayrange,T)(M,ky,&min_ky,&max_ky);
  ...
  if (ndims>2) {
    T min_kz,max_kz;
    TEMPLATE(arrayrange,T)(M,kz,&min_kz,&max_kz);
