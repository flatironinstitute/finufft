...
int cufinufft_spread3d(int ms, int mt, int nf1, int nf2, CPX* h_fw, int M, 
...
#endif
	if(opts.method == 5){
		ier = cuspread3d_subprob_prop(nf1,nf2,M,opts,d_plan);
		if(ier != 0 ){
...
int cuspread3d(cufinufft_opts &opts, cufinufft_plan* d_plan)
{
	...
	switch(opts.method)
	{
		case 1:
			{
				cudaEventRecord(start);
				ier = cuspread3d_idriven(nf1, nf2, M, opts, d_plan);
				if(ier != 0 ){
		...
		case 5:
			{
				cudaEventRecord(start);
				ier = cuspread3d_subprob(nf1, nf2, M, opts, d_plan);
				if(ier != 0 ){
...
int cuspread3d_idriven(int nf1, int nf2, int M, const cufinufft_opts opts, 
	cufinufft_plan *d_plan)
{
	cudaEvent_t start, stop;
	cudaEventCreate(&start);
	...
	cudaEventRecord(start);
	if(opts.Horner){
		Spread_3d_Idriven_Horner<<<blocks, threadsPerBlock>>>(d_kx, d_ky, d_c, 
			d_fw, M, ns, nf1, nf2, es_c, es_beta);
	}
	...
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("[time  ] \tKernel Spread_3d_Idriven \t%.3g ms\n", milliseconds);
#endif
	return 0;
}
...
int cuspread3d_subprob_prop(int nf1, int nf2, int M, const cufinufft_opts opts, cufinufft_plan *d_plan)
{
	cudaEvent_t start, stop;
	cudaEventCreate(&start);
	cudaEventCreate(&stop);
	...
	CalcBinSize_noghost_3d<<<(M+1024-1)/1024, 1024>>>(M,nf1,nf2,bin_size_x,bin_size_y,
...
#endif
	cudaEventRecord(start);
	CalcInvertofGlobalSortIdx_3d<<<(M+1024-1)/1024,1024>>>(M,bin_size_x,bin_size_y,numbins[0],
	...
	/* --------------------------------------------- */
	cudaEventRecord(start);
	CalcSubProb_3d<<<(M+1024-1)/1024, 1024>>>(d_binsize,d_numsubprob,maxsubprobsize,numbins[0]*numbins[1]);
#ifdef DEBUG
	int* h_numsubprob;
	...
	MapBintoSubProb_3d<<<(numbins[0]*numbins[1]+1024-1)/1024, 1024>>>(d_subprob_to_bin,
			d_subprobstartpts,
			d_numsubprob,
			numbins[0]*numbins[1]);
...
int cuspread3d_subprob(int nf1, int nf2, int M, const cufinufft_opts opts, cufinufft_plan *d_plan)
{
	cudaEvent_t start, stop;
	cudaEventCreate(&start);
	cudaEventCreate(&stop);
