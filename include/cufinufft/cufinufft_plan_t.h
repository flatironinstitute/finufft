...
/* This is a POD type containing all the components of a cufinufft_plan_t
that are typically needed on the GPU. It serves as a convenient "bag of bits"
to transport relevant plan data to the device whenever needed. */

template<typename T> struct gpu_plan {
  CUFINUFFT_BIGINT M                          = 0;
  cuda::std::array<CUFINUFFT_BIGINT, 3> nf123 = {0, 0, 0};
  ...
  gpu_plan(const cufinufft_plan_t<T> &p)
    : M(p.m), nf123(p.nf123), totalnumsubprob(p.totalnumsubprob), kxyz(p.kxyz),
      c(p.c), fw(p.fw), N(p.N), nf(p.nf), idxnupts(dethrust(p.idxnupts)), sortidx(p.sortidx),
      numsubprob(p.numsubprob), binsize(p.binsize), binstartpts(p.binstartpts),
      subprob_to_bin(p.subprob_to_bin), subprobstartpts(p.subprobstartpts)
    {}
};
