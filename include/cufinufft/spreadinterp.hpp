...
template<typename T, int ndim>
void cuspread_subprob_and_OD_prop(cufinufft_plan_t<T> &d_plan) {
...
template<typename T, int ndim>
void cuspread3d_blockgather_prop(cufinufft_plan_t<T> &d_plan) {
  if constexpr (ndim == 3) {
...
/* ------------------------------------------------------------------------
   Per-method entry points — CPU pattern (see include/finufft/spread.hpp,
   FINUFFT_PLAN_T<TF>::spread_subproblem_1d/_2d/_3d). Each method becomes a
   class template with static exec/prep members; Ns is dispatched compile-
   time *inside* exec() via finufft::common::dispatch(). Bodies and the
   per-method explicit instantiations live in the per-method TUs
   (src/cuda/spread_*.cu, src/cuda/interp_*.cu). One
   `template struct X<T, Ndim>;` line in each TU instantiates *all*
   members of the class for that <T, Ndim> — same trick used by
   src/fft.cpp::Finufft_FFT_plan_deleter.
   ------------------------------------------------------------------------ */

template<typename T, int Ndim> struct cuspread_nupts_driven_op {
  static void exec(const cufinufft_plan_t<T> &p, const cuda_complex<T> *c,
                   cuda_complex<T> *fw, int blksize);
  static void prep(cufinufft_plan_t<T> &p);
};

template<typename T, int Ndim> struct cuspread_subprob_op {
  static void exec(const cufinufft_plan_t<T> &p, const cuda_complex<T> *c,
                   cuda_complex<T> *fw, int blksize);
  // Prep is shared with output-driven (gpu_method = 3); both call the same prop.
  static void prep(cufinufft_plan_t<T> &p);
};

template<typename T, int Ndim> struct cuspread_output_driven_op {
  static void exec(const cufinufft_plan_t<T> &p, const cuda_complex<T> *c,
                   cuda_complex<T> *fw, int blksize);
  // prep is borrowed from cuspread_subprob_op<T, Ndim>::prep
};

template<typename T> // 3D-only
struct cuspread_3d_blockgather_op {
  static void exec(const cufinufft_plan_t<T> &p, const cuda_complex<T> *c,
                   cuda_complex<T> *fw, int blksize);
  static void prep(cufinufft_plan_t<T> &p);
};

template<typename T, int Ndim> struct cuinterp_nupts_driven_op {
  static void exec(const cufinufft_plan_t<T> &p, cuda_complex<T> *c,
                   const cuda_complex<T> *fw, int blksize);
};

template<typename T, int Ndim> struct cuinterp_subprob_op {
  static void exec(const cufinufft_plan_t<T> &p, cuda_complex<T> *c,
                   const cuda_complex<T> *fw, int blksize);
};

/* ------------------------------------------------------------------------
   Dispatchers — runtime (gpu_method, Ndim) → per-method entry. Ns dispatch
   is internal to each entry, mirroring CPU.
   ------------------------------------------------------------------------ */
...
struct SpreadPropDispatcher {
  template<int Ndim, typename T> void operator()(cufinufft_plan_t<T> &d_plan) {
    ...
    case 1:
      cuspread_nupts_driven_op<T, Ndim>::prep(d_plan);
      return;
    case 2:
    case 3:
      cuspread_subprob_op<T, Ndim>::prep(d_plan);
      return;
    case 4:
      if constexpr (Ndim == 3)
        cuspread_3d_blockgather_op<T>::prep(d_plan);
      else
...
struct SpreadDispatcher {
  template<int Ndim, typename T>
  void operator()(const cufinufft_plan_t<T> &p, const cuda_complex<T> *c,
                  cuda_complex<T> *fw, int blksize) const {
    switch (p.opts.gpu_method) {
    case 1:
      return cuspread_nupts_driven_op<T, Ndim>::exec(p, c, fw, blksize);
    case 2:
      return cuspread_subprob_op<T, Ndim>::exec(p, c, fw, blksize);
    case 3:
      return cuspread_output_driven_op<T, Ndim>::exec(p, c, fw, blksize);
    case 4:
      if constexpr (Ndim == 3) {
        return cuspread_3d_blockgather_op<T>::exec(p, c, fw, blksize);
      } else {
        throw int(FINUFFT_ERR_METHOD_NOTVALID);
      }
    default:
...
struct InterpDispatcher {
  template<int Ndim, typename T>
  void operator()(const cufinufft_plan_t<T> &p, cuda_complex<T> *c,
                  const cuda_complex<T> *fw, int blksize) const {
    switch (p.opts.gpu_method) {
    case 1:
      return cuinterp_nupts_driven_op<T, Ndim>::exec(p, c, fw, blksize);
    case 2:
      return cuinterp_subprob_op<T, Ndim>::exec(p, c, fw, blksize);
    default:
