...
/*
  ...
  2) this routine picks the direction and hands the whole batch to spreadinterpTiled
  Barnett 5/19/20, based on Malleo 2019.
  ChaithyaGR 1/7/25: new arg fwBatch (won't be p.fwBatch if spreadinterponly=1)
*/
{
  // Both directions fold the batch loop into the loop over subprobs, so it is
  // (vector, subprob) pairs that get assigned to threads and all threads stay busy
  // whatever ntr is. Only spreading writes the fine grid, so only spreading can collide,
  // and then only between threads on the same vector's grid.
  if ((m.spopts.spread_direction == 1) != adjoint)
    spreadinterpTiled<1>(reinterpret_cast<T *>(fwBatch), reinterpret_cast<T *>(cBatch),
                         batchSize);
  else
    spreadinterpTiled<2>(reinterpret_cast<T *>(fwBatch), reinterpret_cast<T *>(cBatch),
                         batchSize);
  return 0;
}
