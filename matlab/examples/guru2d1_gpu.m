...
type = 1;
n_modes = [N;N];      % n_dims inferred from length of this
ntrans = 1;
...
y = pi*(2*rand(1,M)-1);                         % choose NU points
c = randn(1,M*ntrans)+1i*randn(1,M*ntrans);     % choose stack of strengths

xg = gpuArray(x); yg = gpuArray(y); cg = gpuArray(c);                % move to GPU
...
plan.setpts(x,y);                                 % send in NU pts
