...
% note that isvector([]) is false, isgpuarray([]) is false, isgpuarray(gpuArray([])) if true.
if ~isvector(x), error('FINUFFT:badXshape','FINUFFT x must be a vector'); end
if isgpuarray(x) ~= is_gpuarray, error('FINUFFT:badDataDevice', 'input data must be on the specified device'); end
nj = numel(x);
if type==3
  nk = numel(s);
  if ~isvector(s), error('FINUFFT:badSshape','FINUFFT s must be a vector'); end
  if isgpuarray(s) ~= is_gpuarray, error('FINUFFT:badDataDevice', 'input data must be on the specified device'); end
else
  nk = 0;   % dummy output
end
if dim>1
  if ~isvector(y), error('FINUFFT:badYshape','FINUFFT y must be a vector'); end
  if isgpuarray(y) ~= is_gpuarray, error('FINUFFT:badDataDevice', 'input data must be on the specified device'); end
  ...
  if type==3
    ...
    if isgpuarray(t) ~= is_gpuarray, error('FINUFFT:badDataDevice', 'input data must be on the specified device'); end
...
if dim>2
  ...
  if isgpuarray(z) ~= is_gpuarray, error('FINUFFT:badDataDevice', 'input data must be on the specified device'); end
  ...
  if type==3
    ...
    if isgpuarray(u) ~= is_gpuarray, error('FINUFFT:badDataDevice', 'input data must be on the specified device'); end
