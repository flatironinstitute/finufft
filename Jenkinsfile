...
// All four images in one podman pod: there is a single image builder, so
// building them in parallel only contends for it and costs an extra checkout
// each. The context is the Dockerfile's own directory, not the repo (nothing is
// COPYed in).
def images = configs.collect { cfg -> [
  context: 'tools/cufinufft/docker',
  ...
  runPod(tag: "cuda${cfg.cuda}",
    ...
    stage("cuda ${cfg.cuda}") {
      withEnv([
        ...
        // compiled, unlike the released all-arch wheels. catchError so a broken
        // wheel still gets archived for inspection.
...
// catchError, not try/finally: it sets currentBuild.result before emailFailure()
// looks at it, which a propagating exception would not have done yet.
catchError {
