...
for i in range(len(cmdStrings)):
    ...
    newSort = stm.sumAllTime(
        "(.*finufft_setpts.*sort)(.*)", strOut
    )  # if type three, adds together both sorts
    ...
    totalOldSpread = stm.sumAllTime(
        "(.*spread.*ier)(.*)", strOut
    )  # gets spread AND unspread (i.e. interpolation) for all trials
