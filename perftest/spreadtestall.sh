...
# as possible. Takes around 1-2 seconds wall-clock time on multicore laptop.
# No math test is done (human could check "rel err" outputs small on right
# ends of lines),
# since this is based on a speed-testing executable (spreadtestnd).
# For pass-fail math tests instead see ../test/finufft*test
# Barnett 10/23/20. Barnett Updated to v2.5 opts 1/12/26.

M=1e6       # medium problem size (# NU pts)
N=1e6       # medium num U grid pts
...
echo "=========== default kernel choice & upsampfac ============"
TOL=1e-6    # req precision
./spreadtestnd 1 $M $N $TOL
./spreadtestnd 2 $M $N $TOL
./spreadtestnd 3 $M $N $TOL
echo "=========== nonstandard upsampfac ========================"
# nonstandard upsampfac to test with the direct kernel eval (slower)...
UP=1.5
# debug output?
DEB=0
./spreadtestnd 1 $M $N $TOL 2 $DEB 0 $UP
./spreadtestnd 2 $M $N $TOL 2 $DEB 0 $UP
./spreadtestnd 3 $M $N $TOL 2 $DEB 0 $UP
...
echo "=========== nonstandard upsampfac ========================"
./spreadtestndf 1 $M $N $TOL 2 $DEB 0 $UP
./spreadtestndf 2 $M $N $TOL 2 $DEB 0 $UP
./spreadtestndf 3 $M $N $TOL 2 $DEB 0 $UP
