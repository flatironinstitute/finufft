...
def plot_stacked_bar_chart(pivot_data, speedup_data, args, figname):
    ...
    for i, version in enumerate(versions):
        speedup = speedup_data[
            (speedup_data["version"] == version)
            & (speedup_data["event"] == "amortized")
        ]["speedup"].values[0]
        plt.text(i, bottom[i], f"{speedup:.2f}x", ha="center", va="bottom")
    text_height = (
        max([bottom[i] for i in range(len(versions))]) * 0.05
...
for version in versions:
    for fft in fft_lib:
        ...
        run_command(
            "cmake",
            ["--build", "build", "-j", str(os.cpu_count()), "--target", "perftest"],
        )
        time.sleep(30)
            ...
            for type in transform:
                ...
                for upsampfac in upsamp:
                    ...
                    if param.thread == 1:
                        out, _ = run_command(
                            "taskset",
                            ["-c", "0", "build/perftest/perftest"] + build_args(args),
                        )
                    else:
                        time.sleep(30)
                        out, _ = run_command(
                            "build/perftest/perftest", build_args(args)
                        )
...
for param in ParamList:
    ...
    for upsampfac in upsamp:
        ...
        for type in transform:
            ...
            baseline_amortized = baseline[baseline["event"] == "amortized"][
                "min(ms)"
            ].values[0]
            # calculate the speedup for all the other versions
            this_data["speedup"] = (
                baseline_amortized
                / this_data[this_data["event"] == "amortized"]["min(ms)"]
            )
            # pivot the data
...
for type in transform:
    ...
    # calculate the speedup for all the other versions
    this_data["speedup"] = (
        baseline_amortized / this_data[this_data["event"] == "amortized"]["min(ms)"]
    )
    # pivot the data
    pivot = this_data.pivot(index="version", columns="event", values="min(ms)")
    pivot = pivot.drop(columns="amortized")
