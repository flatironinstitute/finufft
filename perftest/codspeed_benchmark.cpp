// CodSpeed regression benchmark for FINUFFT.
//
// This executable is run by the CodSpeed CI workflow on an instrumented /
// simulated CPU (cachegrind-style, ~100x slower than bare metal). Its purpose
// is to catch *per-commit performance regressions* in the hot loops
// (spread/interp, FFT, deconvolve) — not to measure absolute throughput.
//
// Because every instruction is simulated, the problem sizes are kept small
// (N, M ~ 1e5) with density ~ 1 (M ~ N), the regime where spread/interp
// and FFT costs are balanced and most representative of real workloads. The
// matrix covers dims {1,2,3} x types {1,2,3} x precision {float,double} = 18
// benchmarks. We let finufft auto-select the upsampling factor from the
// requested tolerance (opts.upsampfac = 0), exercising the realistic default
// path. The full makeplan + setpts + execute + destroy pipeline is timed.
...
// Density ~ 1 (M ~ N) across all three dimensions; sizes kept small so each
// benchmark completes in a few seconds under CodSpeed instrumentation
// (~100x slower than bare metal). tol matches perftest/perftest_config.py:
// 1e-5 for float, 1e-9 for double.
static Params float_benchmarks[] = {
    {std::make_tuple(100000, 1, 1), 1, 100000, 1e-5}, // 1D, N = 100000
    {std::make_tuple(316, 316, 1), 1, 100000, 1e-5},  // 2D, N = 99856
    {std::make_tuple(46, 46, 46), 1, 100000, 1e-5},   // 3D, N = 97336
};
static Params double_benchmarks[] = {
    {std::make_tuple(100000, 1, 1), 1, 100000, 1e-9}, // 1D, N = 100000
    {std::make_tuple(316, 316, 1), 1, 100000, 1e-9},  // 2D, N = 99856
    {std::make_tuple(46, 46, 46), 1, 100000, 1e-9},   // 3D, N = 97336
};
...
template<typename T> void register_benchmark(Params test_opts) {
  for (auto &type : types) {
    ...
    benchmark::RegisterBenchmark(benchmark_name.str(), [=](benchmark::State &state) {
      ...
      opts.upsampfac = 0; // auto-select sigma from tol (realistic default path)
      opts.nthreads  = 1;
      opts.showwarn  = 0;
      for (auto _ : state) {
