    ...
    for dim in range(1, 2):
        ...
        for i in range(1, max_range):
            ...
            for method in ["2", "1"]:
                ...
                setpts = dt[dt["event"].str.contains("setpts")][
                    "nupts/s"
                ].sum()  # it is only one row it extracts the value
                exec = dt[dt["event"].str.contains("exec")][
                    "nupts/s"
                ].sum()  # it is only one row it extracts the value
                ...
                total_fft = dt[
                    dt["Name"].str.contains("fft")
                    & ~dt["Name"].str.contains("cufinufft")
                ]["Duration (ns)"].sum()
...
min_val = min(
    pivot_df[("throughput", "SM")].min(), pivot_df[("throughput", "GM")].min()
)
max_val = max(
    pivot_df[("throughput", "SM")].max(), pivot_df[("throughput", "GM")].max()
)
print(min_val, max_val)
plt.ylim(min_val * 0.90, max_val * 1.1)
# plt.ylim(.8, 1.2)
