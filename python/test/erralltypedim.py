...
def erralltypedim(
    ...
    """Returns (err, info, tolok). tolok=False if eps tolerance unachievable."""
    ...
    try:
        ...
        if "eps tolerance too small" in str(e):
            return err, info, False
        raise


def _erralltypedim_body(
    M,
    Ntot,
    ntr,
    isign,
    ...
    if dims is None or bool(dims[0]):
        ...
        err[0, 0] = np.linalg.norm(f.ravel() - fe_T.ravel()) / np.linalg.norm(
            fe_T.ravel()
        )
        ...
        fe3 = (np.exp(1j * isign * np.outer(s, x)).dot(c.T)).T.astype(
            complex_dtype
        )  # (ntr, M)
    ...
    if dims is None or bool(dims[1]):
        ...
        err[0, 1] = np.linalg.norm(f2.ravel() - fe2.ravel()) / np.linalg.norm(
            fe2.ravel()
        )
        ...
        fe23 = (
            np.exp(1j * isign * (np.outer(s, x) + np.outer(t, y))).dot(c.T)
    ...
    if dims is None or bool(dims[2]):
        ...
        kzv = KZ.ravel()
        A3 = np.exp(
            1j * isign * (np.outer(kxv, x) + np.outer(kyv, y) + np.outer(kzv, z))
        )  # (N1*N2*N3, M)
        f3d = finufft.nufft3d1(
            np.ascontiguousarray(x, dtype=real_dtype),
            np.ascontiguousarray(y, dtype=real_dtype),
        ...
        err[0, 2] = np.linalg.norm(f3d.ravel() - fe3d.ravel()) / np.linalg.norm(
            fe3d.ravel()
        )
        ...
        fe33 = (
            np.exp(1j * isign * (np.outer(s, x) + np.outer(t, y) + np.outer(u, z))).dot(
                c.T
            )
        ).T.astype(complex_dtype)
        err[2, 2] = np.linalg.norm(f33 - fe33) / np.linalg.norm(fe33)

    return err, info, True


if __name__ == "__main__":
    e, info, tolok = erralltypedim(
        100,
        300,
        1,
        +1,
    ...
    print("err shape:", e.shape, "Nmax:", info["Nmax"], "tolok:", tolok)
