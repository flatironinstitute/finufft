...
strt = time.time()
F = finufftpy.nufft1d1(x, c, N, eps=acc, isign=iflag, debug=1, spread_debug=1)
...
strt = time.time()
finufftpy.nufft1d1(x, c, out=F, eps=acc, isign=iflag, modeord=1)
...
strt = time.time()
Ftest2 = finufftpy.nufft1d1(x, c, N, F, acc, iflag, upsampfac=1.25)
print(Ftest2 is F)
